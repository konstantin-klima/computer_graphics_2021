...
class ConcaveCollider
{
  ...
  public:
	ConcaveCollider(const std::vector<Mesh> meshes,
	...
	rp3d::ConcaveMeshShape* getShape() const { return collider; }

  private:
	rp3d::TriangleMesh* createTriangleMesh(std::vector<Mesh> meshes,
										   rp3d::PhysicsCommon& physicsCommon)
	{
		...
		for (const Mesh& mesh : meshes) {
			...
			auto triangleArray = new rp3d::TriangleVertexArray(
				mesh.vertices.size(), (const void*)&mVertices.back()[0],
				3 * sizeof(float), (const void*)&mNormals.back()[0],
				3 * sizeof(float), mesh.numFaces,
				(const void*)&mIndices.back()[0], 3 * sizeof(int),
