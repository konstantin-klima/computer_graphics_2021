...
class Model
{
  ...
  private:
	...
	Mesh processMesh(aiMesh* mesh, const aiScene* scene)
	{
		...
		loadTextureMaterial(material, aiTextureType_DIFFUSE, "texture_diffuse",
							textures);

		loadTextureMaterial(material, aiTextureType_SPECULAR,
							"texture_specular", textures);

		loadTextureMaterial(material, aiTextureType_NORMALS, "texture_normal",
							textures);
							 ...
							 std::vector<Texture>& textures)
	{
		...
		for (unsigned int i = 0; i < mat->GetTextureCount(type); ++i) {
			...
			for (unsigned int j = 0; j < loaded_textures.size(); ++j) {
				if (std::strcmp(str.C_Str(), loaded_textures[i].path.c_str()) ==
					0) {
