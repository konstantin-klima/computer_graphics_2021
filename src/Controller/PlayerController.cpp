...
void PlayerController::init(rp3d::PhysicsCommon* physicsCommon,
							rp3d::PhysicsWorld* world)
{
	...
	player1->addComponent<ShaderComponent>(
		ShaderManager::getManager().getShader("player"));
	...
	p1Body->addCollider(
		player1->getComponent<CapsuleColliderComponent>()->getShape());
	p1Body->getBody()->getCollider(0)->setCollisionCategoryBits(
		Settings::CollisionCategory::PLAYER1 |
	...
	player2->addComponent<ShaderComponent>(
		ShaderManager::getManager().getShader("player"));
	...
	p2Body->addCollider(
		player2->getComponent<CapsuleColliderComponent>()->getShape());
	p2Body->getBody()->getCollider(0)->setCollisionCategoryBits(
		Settings::CollisionCategory::PLAYER2 |
		Settings::CollisionCategory::PLAYER);
...
void PlayerController::update()
{
	...
	for (auto player : players) {
		...
		player->getComponent<MovementComponent>()->moveCollisionBody(
			body, PhysicsController::getDeltaTime());
		auto newPos = body->getPosition();
		player->getComponent<CameraComponent>()->setCameraPos(
...
void PlayerController::processInput(GLFWwindow* window)
{
	...
	for (auto player : players) {
		...
		if (player->getComponent<CameraComponent>()->camIndex == 0) {
			...
			if (p1_currentKeyState_f == GLFW_RELEASE &&
				p1_lastKeyState_f == GLFW_PRESS) {
				switchFlashlight(player);
			}
			p1_lastKeyState_f = p1_currentKeyState_f;

			auto p1_currentKeyState_1 = glfwGetKey(window, GLFW_KEY_1);
			if (p1_currentKeyState_1 == GLFW_RELEASE &&
				p1_lastKeyState_1 == GLFW_PRESS)
...
void PlayerController::castSpell(Entity* player, SPELL spell)
{
	...
	// Make an Entity. It's inside EntityManager, the pointer is here for any
	// additional changes
...
void PlayerController::switchFlashlight(Entity* player)
{
	if (!player->hasComponent<LightComponent>()) {
		...
		auto lc = LightComponent(
			...
			std::forward<glm::vec3>(camera.Front), glm::vec3(1.f, 1.0f, 0.f),
			glm::vec3(0, 1.f, 0.f), glm::vec3(0, 1.f, 0.f),
			glm::cos(glm::radians(12.5f)), glm::cos(glm::radians(15.0f)), 1.0f,
			0.09f, 0.032f);
