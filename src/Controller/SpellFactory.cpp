...
{
	// Some components are referencing others so the order of these calls is
	// important
	makeLightComponent();
...
void SpellFactory::makeLightComponent()
{
	if (m_spell == VOID_BOLT) {
		auto lc = LightComponent(std::forward<glm::vec3>(m_camera->Position),
								 glm::vec3(5, 1, 1), glm::vec3(2.4, 2.3, 1.4),
								 glm::vec3(1.0, 1.0, 1.0), 1.0f, 0.09f, 0.032f);
		m_spellEntity->addComponent<LightComponent>(lc);
	} else if (m_spell == SPECTRAL_ARROW) {
		auto lc = LightComponent(std::forward<glm::vec3>(m_camera->Position),
								 glm::vec3(2, 2, 3), glm::vec3(5.4, 1.3, 6.4),
								 glm::vec3(1.0, 1.0, 1.0), 1.0f, 0.09f, 0.032f);
		m_spellEntity->addComponent<LightComponent>(lc);
	} else if (m_spell == FROST_BOMB) {
		auto lc = LightComponent(std::forward<glm::vec3>(m_camera->Position),
								 glm::vec3(1, 1, 3), glm::vec3(0.1, 1.3, 6.4),
								 glm::vec3(1.0, 1.0, 1.0), 1.0f, 0.09f, 0.032f);
		m_spellEntity->addComponent<LightComponent>(lc);
	}
}
...
void SpellFactory::makeMovementComponent()
{
	...
	if (m_spell == VOID_BOLT || m_spell == SPECTRAL_ARROW ||
		m_spell == FROST_BOMB) {
		auto mc = MovementComponent(direction.x, direction.y, direction.z, 75);
...
{
	auto shader =
		ShaderComponent(ShaderManager::getManager().getShader("basic"));
...
{
	auto cc =
		SphereColliderComponent(0.5f, PhysicsController::getPhysicsCommon());
...
void SpellFactory::makeRigidBodyComponent()
{
	...
	auto body = RigidBodyComponent(position.x + front.x, position.y + front.y,
								   position.z + front.z, 0, 0,
								   PhysicsController::getWorld());
	body.getRigidBody()->enableGravity(false);
	auto linearVelocity =
		m_spellEntity->getComponent<MovementComponent>()->getDirection() *
		m_spellEntity->getComponent<MovementComponent>()->getSpeed();
	body.getRigidBody()->setLinearVelocity(linearVelocity);
	body.addCollider(
		m_spellEntity->getComponent<SphereColliderComponent>()->getShape());
	// Allow the spell to only collide with the arena and the enemy player
	body.getRigidBody()->getCollider(0)->setCollisionCategoryBits(
		Settings::CollisionCategory::SPELL);
	Settings::CollisionCategory enemyCategory;
	if (m_player->getComponent<CollisionBodyComponent>()
			->getBody()
			->getCollider(0)
