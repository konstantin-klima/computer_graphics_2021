...
struct Component {
  ...
  public:
	template <typename T> static ComponentTypeID GetComponentTypeID()
	{
...
enum LIGHTS { DIRECT, POINT, SPOTLIGHT };
...
struct LightComponent : public Component {
	...
	LightComponent(glm::vec3&& position, glm::vec3&& ambient,
	...
	LightComponent(glm::vec3&& direction, glm::vec3&& ambient,
	...
	LightComponent(glm::vec3&& position, glm::vec3&& direction,
				   glm::vec3&& ambient, glm::vec3&& diffuse,
				   glm::vec3&& specular, float cutOff, float cutOffOuter,
...
struct CameraComponent : public Component {
	CameraComponent(glm::vec3 pos, unsigned index)
		: camIndex(index)
	{
		camera = Camera(pos);
	}

	CameraComponent(Camera camera, unsigned index)
		: camera(camera)
		, camIndex(index) {};

	CameraComponent(float x, float y, float z, unsigned index)
		: camIndex(index)
	{
	...
	void setCameraPos(glm::vec3 pos) { camera.Position = pos; }
	...
	[[nodiscard]] glm::vec3 getCameraFront() const { return camera.Front; }
...
struct ShaderComponent : public Component {
	...
	explicit ShaderComponent(Shader* s)
		: shader(s) {};
...
struct ModelComponent : public Component {
	...
	std::vector<Mesh> getMeshes() const { return model->meshes; }
...
struct PlayerModelComponent : public Component {
	PlayerModelComponent() { model = new PlayerModel(); }

	PlayerModel* getModel() const { return model; }

	void draw(Shader* shader) { model->draw(shader); }

	void setAlive(bool alive) { model->setAlive(alive); }
...
struct SkyboxComponent : public Component {
	...
	void setModel(Skybox skybox) { this->skybox = skybox; }
...
struct RigidBodyComponent : public Component {
	RigidBodyComponent(rp3d::RigidBody* body)
		: body(body) {};

	RigidBodyComponent(float x, float y, float z, float pitch, float yaw,
					   rp3d::PhysicsWorld* world, bool env = false)
	{
	...
	void setMass(float mass) { body->setMass(mass); }

	float getMass() const { return body->getMass(); }

	rp3d::RigidBody* getRigidBody() const { return body; }
	...
	{
		auto newOrientation =
			rp3d::Quaternion().fromEulerAngles(pitch, yaw, roll);
	...
	rp3d::Transform getTransform() const { return body->getTransform(); }
...
struct CollisionBodyComponent : public Component {
	CollisionBodyComponent(float x, float y, float z, float yaw, float pitch,
						   rp3d::PhysicsWorld* world, bool gravity = true,
						   bool isFalling = true)
	{
		auto pos = rp3d::Vector3(x, y, z);
		auto orientation = rp3d::Quaternion::fromEulerAngles(yaw, pitch, 0);
	...
	rp3d::CollisionBody* getBody() const { return this->body; }
	...
	bool affectedByGravity() const { return gravity; }
	...
	glm::mat4 getGLMTransform()
	{
		...
		return glm::mat4({{transformArr[0], transformArr[1], transformArr[2],
						   transformArr[3]},
						  {transformArr[4], transformArr[5], transformArr[6],
						   transformArr[7]},
						  {transformArr[8], transformArr[9], transformArr[10],
						   transformArr[11]},
						  {transformArr[12], transformArr[13], transformArr[14],
						   transformArr[15]}});
	}
...
struct MovementComponent : public Component {
	MovementComponent(float dirx, float diry, float dirz, float speed)
		: direction(rp3d::Vector3(dirx, diry, dirz))
		, speed(speed) {};

	explicit MovementComponent(rp3d::Vector3* dir, float speed)
		: direction(rp3d::Vector3(dir->x, dir->y, dir->z))
		, speed(speed) {};
	...
	rp3d::Vector3 getDirection() const { return direction; }
...
enum SPELL_TYPES { BULLET, PARABOLA };
...
struct HealthComponent : public Component {
	...
	{
		// If negative, unit is immortal (you cannot kill that which has no
		// life)
...
struct CapsuleColliderComponent : public Component {
	...
	rp3d::CapsuleShape* getShape() const { return shape; }
...
struct BoxColliderComponent : public Component {
	BoxColliderComponent(float halfX, float halfY, float halfZ,
						 rp3d::PhysicsCommon* physicsCommon)
	{
		shape =
			physicsCommon->createBoxShape(rp3d::Vector3(halfX, halfY, halfZ));
	}

	rp3d::BoxShape* getShape() const { return shape; }
...
struct SphereColliderComponent : public Component {
	...
	rp3d::SphereShape* getShape() const { return shape; }
...
struct ConcaveColliderComponent : public Component {
	ConcaveColliderComponent(std::vector<Mesh> meshes,
							 rp3d::PhysicsCommon* physicsCommon)
	{
		collider = new ConcaveCollider(meshes, *physicsCommon);
	}

	rp3d::ConcaveMeshShape* getShape() const { return collider->getShape(); }
