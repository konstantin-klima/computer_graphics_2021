...
TEST_CASE("Given Entity with Components of type TestComponent1, EntityManager returns it") {
    ...
    REQUIRE(EntityManager::getManager().getEntitiesWithComponent<TestComponent1>().size() == 1);
}

TEST_CASE("Given Entities with different components, getAllComponents returns the right ones") {
    EntityManager::getManager().clearEntities();
    ...
    REQUIRE(EntityManager::getManager().getAllComponents<TestComponent2>().size() == 2);
}